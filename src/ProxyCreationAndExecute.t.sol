...
contract ProxyCreationAndExecuteTest is DSTest {
    ...
    function testProxySellAll() public {
        ...
        (address payable proxy, uint buyAmt) = creator.createAndSellAllAmount(factory, OtcInterface(address(otc)), TokenInterface(address(mkr)), 4000 ether, TokenInterface(address(weth)), expectedResult);
        uint endGas = gasleft();
    ...
    function testProxySellAllPayEth() public {
        ...
        uint startGas = gasleft();
        (address payable proxy, uint buyAmt) = creator.createAndSellAllAmountPayEth.value(15 ether)(factory, OtcInterface(address(otc)), TokenInterface(address(mkr)), expectedResult);
        uint endGas = gasleft();
        emit log_named_uint('Gas', startGas - endGas);
        assertEq(buyAmt, expectedResult);
    ...
    function testProxySellAllBuyEth() public {
        ...
        uint expectedResult = 10 ether * 2800 / 2800 + 10 ether * 1200 / 3200;
        uint startGas = gasleft();
        (address payable proxy, uint buyAmt) = creator.createAndSellAllAmountBuyEth(factory, OtcInterface(address(otc)), TokenInterface(address(mkr)), 4000 ether, expectedResult);
        uint endGas = gasleft();
        emit log_named_uint('Gas', startGas - endGas);
        assertEq(buyAmt, expectedResult);
    ...
    function testProxyBuyAll() public {
        ...
        uint startGas = gasleft();
        (address payable proxy, uint payAmt) = creator.createAndBuyAllAmount(factory, OtcInterface(address(otc)), TokenInterface(address(weth)), 15 ether, TokenInterface(address(mkr)), expectedResult);
        uint endGas = gasleft();
        emit log_named_uint('Gas', startGas - endGas);
    ...
    function testProxyBuyAllPayEth() public {
        ...
        uint startGas = gasleft();
        (address payable proxy, uint payAmt) = creator.createAndBuyAllAmountPayEth.value(expectedResult)(factory, OtcInterface(address(otc)), TokenInterface(address(mkr)), 4600 ether);
        uint endGas = gasleft();
        emit log_named_uint('Gas', startGas - endGas);
        assertEq(payAmt, expectedResult);
    ...
    function testProxyBuyAllBuyEth() public {
        ...
        uint expectedResult = 2800 ether * 10 / 10 + 3200 ether * 5 / 10;
        uint startGas = gasleft();
        (address payable proxy, uint payAmt) = creator.createAndBuyAllAmountBuyEth(factory, OtcInterface(address(otc)), 15 ether, TokenInterface(address(mkr)), expectedResult);
        uint endGas = gasleft();
        emit log_named_uint('Gas', startGas - endGas);
        assertEq(payAmt, expectedResult);
        assertEq(address(this).balance, initialBalance + 15 ether); // Balance ETH
    ...
    function testFailSendFunds() public {
        (bool success,) = address(creator).call.value(1 ether)("");
        require(success, "");
    }
